...
<body>
    ...
    <div class="flex justify-center font-bold text-2xl mt-20">UNBLOCK USER</div>
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_stmt_num_rows($stmt) > 0) {
        ...
        if($admin == FALSE){
            if($block == TRUE){
                $blockquery = "UPDATE USERS SET Block = False WHERE Email = ?";
            ...
            echo "<script>
                alert('You cannot unblock another Admin')
            </script>";
        }
    } else {
        // User does not exist
